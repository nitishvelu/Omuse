...
const incrementStreams = (song_id, artist_id) => {
	const doc_ref = firebase.firestore().collection("song").doc(song_id);
	doc_ref.update({ no_of_streams: FieldValue.increment(1) });

	const artist_ref = firebase.firestore().collection("artist").doc(artist_id);
	artist_ref.update({ total_streams: FieldValue.increment(1) });
};
				...
				<Link>
					<div
						...
						onClick={function clickHandler(e) {
							...
							//media sesion api for notification player
							navigator.mediaSession.metadata =
								new window.MediaMetadata({
									title: song_obj.name.replace(
										/(?:^|\s|["'([{])+\S/g,
										(match) => match.toUpperCase()
									),
									artist: song_obj.artist_name.replace(
										/(?:^|\s|["'([{])+\S/g,
										(match) => match.toUpperCase()
									),
									album: "Omuse",
