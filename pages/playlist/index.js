...
function Playlist({ playlist_details }) {
  ...
  function BasicUsage() {
    ...
    return (
      <>
        <Button onClick={onOpen} leftIcon={<FiPlusCircle />} variant="outline">
          Create New Playlist
        </Button>
            ...
            <ModalFooter>
              ...
              <Button
                onClick={(e) => {
                  ...
                  if (match) {
                    console.log(match[2]);
                    let playlist = db.collection("playlist").doc(newPlaylistId);
                    playlist.set({
                      name: value,
                      songs: [],
                      date: today,
                    ...
                    db.collection("user")
                      ...
                      .update({
                        playlists:
                          firebase.firestore.FieldValue.arrayUnion(playlist),
                      });
                  }
                  onClose();
          ...
          <VStack className="stack">
            {Object.keys(playlist_details).map((idx) => {
              return (
                <div key={idx}>
                  <NextLink
                    href={"/playlist/" + playlist_details[idx].id}
                    passHref
                    key={idx}
                  >
                    <Link
